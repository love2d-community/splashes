...
function splashlib.new(init)
  ...
  self.shader:send("radius", math.max(self.width*self.height))
  ...
  self.stripes = {
    ...
    height = 100,
    offset = -2 * self.width,
    radius = math.max(self.width, self.height),
    shadow = 0,
  }
  ...
  timer.script(function(wait)
    ...
    timer.tween(0.3, self.stripes, {rot = -5 * math.pi / 18, height=self.height})
    wait(0.2)
  ...
  self.isCoveringScreen = splashlib.isCoveringScreen
  self.isDone = splashlib.isDone
...
function splashlib:draw()
  ...
  self.canvas:renderTo(function()
    ...
    love.graphics.rectangle("fill",
      self.stripes.offset - self.width,
      -self.stripes.height,
      self.width * 2,
      self.stripes.height)
    ...
    love.graphics.rectangle("line",
      -self.width - self.stripes.offset,
      0,
      self.width * 2,
...
function splashlib:isCoveringScreen()
  return false
end

function splashlib:isDone()
  return self.done
end
